...
class UserAddTempView(LoginRequiredMixin,CreateView):
    fields = ["temperature"]
    template_name = 'user_addTemp_form.html'
    model = Temperatures
    def get_success_url(self):
        return reverse(
            "users:detail",
            kwargs={'username': User.request.user.username},
        )
...
user_addTemp_view = UserAddTempView.as_view()
